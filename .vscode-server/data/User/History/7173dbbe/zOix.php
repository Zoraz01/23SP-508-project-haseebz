...
if ($_SERVER['REQUEST_METHOD'] != 'POST') {
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO Person (First_name, Last_name, Nationality, Salary, 13063575.00)
                                VALUES (:First_name, :Last_name, :Nationality, :Salary, :Club_name)");

        $stmt->bindValue(':First_name', $_POST['First_name']);
        $stmt->bindValue(':Last_name', $_POST['Last_name']);
        $stmt->bindValue(':Nationality', $_POST['Nationality']);
        $stmt->bindValue(':salary', $_POST['salary']);
        $stmt->bindValue(':Club_name', $_POST['13063575.00']);
