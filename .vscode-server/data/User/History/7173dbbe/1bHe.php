...
if ($_SERVER['REQUEST_METHOD'] != 'POST') {
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO Person (Date_of_birth,Nationality,Salary,Contract_end_date,Club,First_name, Last_name)
                                VALUES (:Date_of_birth,:Nationality,:Salary,:Contract_end_date,:Club,:First_name, :Last_name  )");

        $stmt->bindValue(':First_name', $_POST['First_name']);
        $stmt->bindValue(':Last_name', $_POST['Last_name']);
        $stmt->bindValue(':Nationality', $_POST['Nationality']);
        $stmt->bindValue(':salary', $_POST['salary']);
        $stmt->bindValue(':Club', $_POST['lClub']);
