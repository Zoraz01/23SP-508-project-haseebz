...
if (!isset($_GET['Person_id'])) {
    ...
    $Person_id = $_GET["Person_id"]; // GET NOT SAFE FOR PRIVACY OF VARIABLES
    ...
    while ($row = $stmt->fetch()) {
        echo "<tr><td>$row[Person_id]</td><td>$row[First_name]</td><td>$row[Last_name]</td><td>$row[Nationality]</td><td>" . $fmt->formatCurrency(number_format($row["salary"],2), "USD") . "</td><td>$row[Club]</td><td>$row[Contract_end_date]</td></tr>";
    }
