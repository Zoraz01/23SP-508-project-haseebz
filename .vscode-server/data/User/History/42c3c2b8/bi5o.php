...
if (!isset($_GET['Person_id'])) {
    ...
    $stmt = $conn->prepare("SELECT Person_id, First_name, Last_name, Nationality,Salary,Club, FROM Person WHERE Person_id=:Person_id"); // PREPARED STATEMENT SAFE FOR SQL INJECTION
    ...
    while ($row = $stmt->fetch()) {
        echo "<tr><td>$row[Person_id]</td><td>$row[First_name]</td><td>$row[Last_name]</td><td>$row[Nationality]</td><td>" . $fmt->formatCurrency($row["salary"], "USD") . "</td><td>$row[Club]</td></tr>";
    }
