...
if (!isset($_GET['Person_id'])) {
    ...
    while ($row = $stmt->fetch()) {
        echo "<tr><td>$row[Person_id]</td><td>$row[First_name]</td><td>$row[Last_name]</td><td>$row[Nationality]</td><td>$row[Salary]</td><td>" . $fmt->formatCurrency($row["salary"], "USD") ."</td><td>$row[Club]</td><td>$row[Contract_end_date]</td></tr>";
    }
